...
bool isNumeric(const std::string& y) {
	return std::all_of(y.begin(), y.end(), ::isdigit);
}

double ERK( double t_i, double t_f, const int k_max, const int n_max)//write a method to apply explicit Runge-Kutta
{
...
double p_0=0;
    ...
    ofstream outfile;
    outfile.open("output3d.dat");//write out for plot

//interupt with a loop for p0
for (int k=0; k < k_max+1; k = k+1) { 
p_0 = 0.01 + k*0.1/k_max;
cout<< "po : " << p_0<< endl;
...
//std::string ystring = std::to_string(y[2]);

if ( y[2] < 1e-10  ) { cout<< "R: " <<setprecision(15) << t_n << " Mass: " <<setprecision(15) <<y[1]<<"pressure:  "<<setprecision(15)<< y[2]<< endl; 
outfile<< setprecision(15) << t_n<<" "<< setprecision(15) <<y[1]<< endl; 
goto LOOP;    }  
