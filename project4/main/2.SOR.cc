...
{ 
    ...
    double omega=2*(1-sin(M_PI/(J+1)))/ cos(M_PI/(J+1))/  cos(M_PI/(J+1)); cout <<omega<<"---"<<endl; 
   ...
   //ARRAY TO HOLD THE FUNCTION AT TWO CONSECUTIVE TIME 
    ...
    outfile1.open("prob2.SOR.dat");
    outfile2.open("prob2.SOR.error.dat");
         ...
         {  
            for (int l=0;l<L+1;l++){
                if ( (j!=0) && (j!=L) && ( l!=0) && (l!=L) && //IF INTERIOR
                    ...
                    ( (double(l)*dy)  <= (2.75-1.5*dx*double(j)) )    )
                     {  
                         v1[j][l]= (1.0-omega)*v0[j][l] + omega*  ( (v0[j+1][l] + v1[j-1][l]  ) *dy*dy +
...
// final result
    ...
    delete [] error;
    for (int i=0;i<J;i++) delete [] v0[i];
    delete [] v0;
    for (int i=0;i<J;i++) delete [] v1[i];
    delete [] v1;
