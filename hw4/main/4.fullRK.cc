...
{
    const int nu_max =4;//number of step
    double h = (t_f-t_i)/n_max;//step size
    double t_n;//intitial 'time'
    ...
    double y[3]={0,1,0};//initial value. y=(u,v). the first dimension is unused	
   
    ofstream outfile;
    outfile.open("output.dat");//write out for plot
    ...
    for (int n=1; n < n_max+1; n=n+1 ) //phi step
    {
        ...
        for (int i=1; i< nu_max+1 ; i=i+1 ) // loop over all stages to update y_n 
        {
           ...
           {
                F[1][j]=(psi[2][j]);// u_n+1 = v_nv//THE RIGHT HAND SIDE 
                sum_psi[1] = A[i][j]*F[1][j];
                
                F[2][j]=(-psi[1][j] + psi[1][j]*psi[1][j]);//v_n+1 = -u + alpha + beta* u^2
        ...
        for (int i=1;i<nu_max+1; i=i+1) 
            { 
               f[1][i]=psi[2][i];
               y[1] = y[1] +h* b[i]*f[1][i];
               f[2][i]=(-psi[1][i] + alpha + beta * psi[1][i] *psi[1][i]);
               y[2] = y[2] +h* b[i]*f[2][i];
...
outfile <<t_n<<" "<< setprecision(15)<<y[1]<<" " << setprecision(15)<<   newton_u<<" "<< y[1]-newton_u <<endl;
        cout<< "t_"<<n<<": "<<setprecision(15)<<t_n<<" ";
        cout << "RK_U_"<<setprecision(15)<<n<<": "<< y[1];
